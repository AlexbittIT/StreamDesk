 ...
 *
 * NB: это слой ИИ-сборки (SD-152/157). Хранение смет и версий (SD-165) — в
 * {@link EstimateService}; серверный расчёт смен эндпоинта /calculate — в
 * {@link ShiftCalculationService}.
 */
@Service
public class EstimateAnalysisService {

    private static final Logger log = LoggerFactory.getLogger(EstimateAnalysisService.class);
    ...
    public EstimateAnalysisService(CatalogService catalogService,
    ...
    private EstimateResult assembleResult(String title, List<EstimateLine> lines, List<CatalogEntry> catalog,
        ...
        if (aiError != null) {
            warnings.add("ИИ недоступен (" + aiError + ") — смета собрана складским и эвристическим подбором.");
        }
        if (!missing.isEmpty()) {
